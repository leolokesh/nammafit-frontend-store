...
export default function Footer({ className = "" }: FooterProps) {
  return (
    <footer className={`w-full py-6 px-8 border-t border-white/5 text-[11px] text-slate-500 tracking-wider flex flex-col md:flex-row items-center justify-between gap-4 select-none ${className}`}>
      {/* Copyright */}
      <div className="font-semibold uppercase order-1">
        © 2026 NAMMAFIT
      </div>

      {/* Diversity Message */}
      <div className="flex items-center gap-1 text-slate-400 font-medium order-2">
      ...
      {/* Links */}
      <div className="flex items-center gap-6 font-semibold uppercase order-3">
        <a
