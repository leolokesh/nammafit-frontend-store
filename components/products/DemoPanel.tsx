    ...
    <div className="conic-border rounded-3xl overflow-hidden shadow-[0_40px_120px_-60px_rgba(176,228,204,0.25)] bg-[#091413]">
      <AnimatePresence mode="wait">
        ...
        {recoStatus === "success" && result && (
          <ResultView key="result" result={result} onTryAgain={goBackToForm} />
        )}
      </AnimatePresence>
    </div>
  );
...
/* ============= RESULT ============= */
function ResultView({ result, onTryAgain }: any) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.28em] text-[#B0E4CC]">
          <span className="h-1.5 w-1.5 rounded-full bg-[#B0E4CC] animate-valar-pulse" />
          NammaFit Engine
        </div>
        <button
          type="button"
